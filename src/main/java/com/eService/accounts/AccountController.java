...
public class AccountController {
	...
	@RequestMapping("/card/{cardno}")
	public String byCardId(@PathVariable("cardno") byte[] cardno) {
		return decrypt(cardno);
	}
	
	public String decrypt(byte[] encryptedText)
	{
	 System.out.println("Rakesh-encrypted text: '" + encryptedText );
	 EncPublicPrivateKey edpkpk = new EncPublicPrivateKey();
	 String decryptedText = new String(edpkpk.decryptText(encryptedText));
	 System.out.println("Account-decrypted text: '" + decryptedText + "'" );
	 System.out.println("Account-decrypted text length: '" + decryptedText.length() + "'");
